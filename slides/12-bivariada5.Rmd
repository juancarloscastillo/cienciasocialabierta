...
pacman::p_load(tidyverse, kableExtra, sjlabelled, sjmisc, gginference)
```
...
.pull-left-narrow[
.medium[
También se puede visualizar el resultado con la función `ggchisqtest`, de librería `gginference`:

```{r eval=FALSE}
chi_results <- chisq.test(table(casen2022_chi$educ_sup,
                 casen2022_chi$discrim))
...
proc_casen$educ_sup <- rec(proc_casen$educ, rec = "0:8=0;9:12=1",val.labels = c("Menos que universitaria", "Universitaria o más"))
proc_casen$pobreza_multi_5d <- set_label(x = proc_casen$pobreza_multi_5d,label = "Pobreza multidimensional") 
```


# Ejercicio de aplicación 1

**¿Existe una asociación entre la pobreza multidimensional y el sexo?**

```{r echo=FALSE}
proc_casen %>%
  sjtab(sexo,pobreza_multi_5d,
        encoding="UTF-8",
        show.summary = FALSE)
```
...
**¿Existe una asociación entre la situación laboral y el sexo?**
